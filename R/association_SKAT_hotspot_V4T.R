  ...
  out <- tryCatch({
    ...
    if ( sum(varframebycase,na.rm = T) >= 6){
      ...
      if (length(unique(samplemetatablewithentity$entity)) == 1){
        obj<-SKAT_Null_Model( samplemetatablewithentity$normalized_exposures ~ samplemetatablewithentity$total_variants + samplemetatablewithentity$gender, out_type="C")
      }else{
        obj<-SKAT_Null_Model( samplemetatablewithentity$normalized_exposures ~ samplemetatablewithentity$entity + samplemetatablewithentity$total_variants + samplemetatablewithentity$gender, out_type="C")
      }
        #}
			...
			#the ChrX mode doesnt seem to be necessary--> test 
			#if (idxchr == 23){
			#  out = SKAT_ChrX(varframebycasemat, obj, weights=weightframencLIST, method="SKATO")
			#}else{
			  out = SKAT(varframebycasemat, obj, weights=weightframencLIST, method="SKATO")
			#}
			  
			#UA(Unadjusted) not allowed, forcing ER-A
			#if (out$method == "UA"){
			#  out = SKAT(varframebycasemat, obj, weights=weightframencLIST, method.bin="QA")
			  #out = SKATBinary(varframebycasemat, obj, weights.beta=c(0.5,0.5), method.bin="QA")
			#}
